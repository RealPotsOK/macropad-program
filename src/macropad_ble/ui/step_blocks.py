...
STEP_BLOCK_PALETTE: list[tuple[str, str, str]] = [
    (BLOCK_REPEAT, "Repeat X Times", "#2563EB"),
    (BLOCK_MOVE_MOUSE, "Move Mouse", "#0F766E"),
    (BLOCK_CLICK_MOUSE, "Click Mouse", "#1D4ED8"),
    (BLOCK_SAVE_MOUSE_POS, "Save Mouse Pos", "#15803D"),
    (BLOCK_RESTORE_MOUSE_POS, "Move To Saved Pos", "#0EA5E9"),
    (BLOCK_TYPE_TEXT, "Type Text", "#9333EA"),
    (BLOCK_HOLD_KEY, "Hold Key", "#C2410C"),
    (BLOCK_WAIT, "Wait (Seconds)", "#475569"),
    (BLOCK_RELEASE_KEY, "Release Key", "#BE123C"),
    (BLOCK_IF_PRESSED, "If Key Pressed", "#0E7490"),
    (BLOCK_IF_ELSE_PRESSED, "If / Else Key", "#7C3AED"),
    (BLOCK_WHILE_PRESSED, "While Key Pressed", "#CA8A04"),
    (BLOCK_END, "End", "#334155"),
]

SCOPED_BLOCKS = {BLOCK_IF_PRESSED, BLOCK_WHILE_PRESSED}
...
def default_step_block(block_type: str) -> dict[str, Any]:
    ...
    if normalized == BLOCK_WHILE_PRESSED:
        return {"type": BLOCK_WHILE_PRESSED, "key": "ctrl", "max_loops": 50, "interval": 0.05}
...
def normalize_step_block(raw: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if block_type == BLOCK_TYPE_TEXT:
        ...
        return block
    if block_type in {BLOCK_HOLD_KEY, BLOCK_RELEASE_KEY, BLOCK_IF_PRESSED, BLOCK_IF_ELSE_PRESSED}:
    ...
    if block_type == BLOCK_WHILE_PRESSED:
        block["key"] = _clean_key(data.get("key"))
        block["max_loops"] = _clamp_int(data.get("max_loops"), 50, minimum=1, maximum=100_000)
        block["interval"] = _clamp_float(data.get("interval"), 0.05, minimum=0.0, maximum=10.0)
        return block
    return block
...
def summarize_step_block(block: dict[str, Any], *, index: int, indent: int = 0) -> str:
    ...
    if block_type == BLOCK_MOVE_MOUSE:
        ...
        detail = f"Repeat next block {normalized['times']}x"
        ...
        detail = (
            f"While [{normalized['key'] or '<key>'}] pressed -> run until End "
            f"(max {normalized['max_loops']}, every {normalized['interval']:.2f}s)"
        )
    ...
    async def run_range(start_index: int, end_index: int) -> None:
        ...
        while index < end_index:
            ...
            if block_type == BLOCK_WHILE_PRESSED:
                ...
                body_end = find_matching_end(body_start)
                key_name = _clean_key(block.get("key"))
                max_loops = _clamp_int(block.get("max_loops"), 50, minimum=1, maximum=100_000)
                interval = _clamp_float(block.get("interval"), 0.05, minimum=0.0, maximum=10.0)
                loops = 0
                while loops < max_loops and _is_key_pressed(key_name):
                    await run_range(body_start, min(body_end, end_index))
                    loops += 1
                    if interval > 0:
