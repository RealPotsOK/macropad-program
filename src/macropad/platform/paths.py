...
APP_DATA_DIRNAME = "macropad"
LEGACY_APP_DATA_DIRNAME = "macropad-ble"
...
class AppPaths:
    ...
    profile_dir: Path
    state_path: Path
    legacy_appdata_root: Path
    legacy_appdata_profile_dir: Path
    legacy_appdata_state_path: Path
...
def resolve_app_paths(
    *,
    ...
    legacy_appdata_root = data_root.with_name(LEGACY_APP_DATA_DIRNAME)
    return AppPaths(
        data_root=data_root,
        profile_dir=data_root / LEGACY_PROFILE_DIRNAME,
        state_path=data_root / APP_STATE_FILENAME,
        legacy_appdata_root=legacy_appdata_root,
        legacy_appdata_profile_dir=legacy_appdata_root / LEGACY_PROFILE_DIRNAME,
        legacy_appdata_state_path=legacy_appdata_root / APP_STATE_FILENAME,
...
def migrate_legacy_app_data(paths: AppPaths) -> bool:
    ...
    sources = [
        (paths.legacy_appdata_profile_dir, paths.legacy_appdata_state_path),
        (paths.legacy_profile_dir, paths.legacy_state_path),
    ]

    for profile_dir, state_path in sources:
        if not profile_dir.exists():
            continue

        migrated = False
        paths.profile_dir.mkdir(parents=True, exist_ok=True)
        for source in profile_dir.iterdir():
            if source.name == APP_STATE_FILENAME:
                continue
            _copy_item(source, paths.profile_dir / source.name)
            migrated = True

        if state_path.exists() and not paths.state_path.exists():
            paths.state_path.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(state_path, paths.state_path)
            migrated = True
