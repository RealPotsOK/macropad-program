...
CONFIG_FILENAME = "macropad.toml"
LEGACY_CONFIG_FILENAME = "macropad-ble.toml"
APP_CONFIG_FILENAME = "config.toml"
APP_CONFIG_DIRNAME = "macropad"
LEGACY_APP_CONFIG_DIRNAME = "macropad-ble"
...
def legacy_user_config_path(
    *,
    ...
    if system == "Windows":
        appdata = env.get("APPDATA")
        if appdata:
            return Path(appdata) / LEGACY_APP_CONFIG_DIRNAME / APP_CONFIG_FILENAME
        return home / "AppData" / "Roaming" / LEGACY_APP_CONFIG_DIRNAME / APP_CONFIG_FILENAME
    ...
    xdg = env.get("XDG_CONFIG_HOME")
    if xdg:
        return Path(xdg) / LEGACY_APP_CONFIG_DIRNAME / APP_CONFIG_FILENAME
    return home / ".config" / LEGACY_APP_CONFIG_DIRNAME / APP_CONFIG_FILENAME
...
def discover_config_path(
    *,
    ...
    legacy_project_path = cwd / LEGACY_CONFIG_FILENAME
    legacy_user_path = legacy_user_config_path(system=system, env=env, home=home)

    for candidate in (project_path, legacy_project_path, user_path, legacy_user_path):
        if candidate.exists():
            return candidate
    return None
